...
function WifiPattern(id, ssid, locked, signal) { result = '<a name="TYPE" onclick="wifiClick(this);" id="ID"><label id="name">NETWORK_NAME</label><img id="signal" src="SIGNAL"/><img id="lock" src="LOCK_ICON"/></a>'.replace("NETWORK_NAME", ssid).replace("ID", id).replace("LOCK_ICON", locked == true ? lockImage : unlockImage).replace("TYPE", locked == true ? "locked" : "unlocked").replace("SIGNAL", String(GetSignal(signal))); return result; }
