        ...
        if(this.readyState == 4 && this.status == 200){
            ...
            if(resp != "None"){
                ...
                wifies.forEach(function(item, index){
                    ...
                    if(wifi_name == ssid) resultString = resultString + WifiPattern(wifi_name, false, wifi_signal);
                    else resultString = resultString + WifiPattern(wifi_name, true, wifi_signal);
                });
...
function wifiClick(element){
    ssid = element.querySelector("#name").innerHTML;
...
function WifiPattern(ssid, locked, signal){result = '<a onclick="wifiClick(this);"><label id="name">NETWORK_NAME</label><img id="signal" src="./imgs/SIGNAL.png"/><img id="lock" src="./imgs/LOCK_ICON.png"/></a>'.replace("NETWORK_NAME", ssid).replace("LOCK_ICON", locked == true ? "lock" : "unlock").replace("SIGNAL", "signal" + String(GetSignal(signal)));return result;}
